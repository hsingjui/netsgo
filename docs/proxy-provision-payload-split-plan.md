...
| system E2E 主 data path 覆盖 | `[GREEN]` | `TestSystemE2E`、`make test-system-e2e-nginx` 和 `make test-system-e2e-caddy` 已通过 | HTTP reverse proxy/direct、HTTP auth、server-expose TCP/UDP/SOCKS5、c2c TCP/UDP/SOCKS5、并发、restart restore 都通过，且 `issues` 为空 |
